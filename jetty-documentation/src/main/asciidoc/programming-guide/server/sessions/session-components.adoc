...
If the `DefaultSessionIdManager` does not meet your needs, you can extend the link:{JDURL}/org/eclipse/jetty/server/session/AbstractSessionIdManager[AbstractSessionIdManager] or do a fresh implementation of the link:{JDURL}/org/eclipse/jetty/server/session/SessionIdManager[SessionIdManager] interface.

See link:#session-configuration-housekeeper[Configuring the SessionIdManager and HouseKeeper] for details on configuration.

==== HouseKeeper

There is a maximum of one (1) link:{JDURL}/org/eclipse/jetty/server/session/HouseKeeper[HouseKeeper] per `SessionIdManager`.
Its purpose is to periodically poll the link:{JDURL}/org/eclipse/jetty/server/session/SessionHandler[SessionHandlers] to clean out expired sessions.
...
Jetty provides two (2) `SessionCache` implementations: the link:{JDURL}/org/eclipse/jetty/server/session/DefaultSessionCache[DefaultSessionCache] and the link:{JDURL}/org/eclipse/jetty/server/session/NullSessionCache[NullSessionCache].
...
`SessionCaches` always write out a Session to the link:{JDURL}/org/eclipse/jetty/server/session/SessionDataStore[SessionDataStore] whenever the last request for the Session exits.
...
There is one (1) link:{JDURL}/org/eclipse/jetty/server/session/SessionDataStore[SessionDataStore] per context.
...
Jetty instantiates the trivial link:{JDURL}/org/eclipse/jetty/server/session/NullSessionDataStore[NullSessionDataStore] - which does not persist sessions - as the default.
...
The `CachingSessionDataStore` is a special type of `SessionDataStore` that inserts a L2 cache of Session data - the link:{JDURL}/org/eclipse/jetty/server/session/SessionDataMap[SessionDataMap] - in front of a delegate `SessionDataStore`.
